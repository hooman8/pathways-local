...
import { createProject, updateTeam, saveProjectTask, selectTaskTeam, saveProjectTemplate } from "../lib/projects";
...
test("templates respect workspace access and survive old or scoped client saves", async () => {
  const service = store();
  let snapshot = await service.get(owner);
  const sourceId = snapshot.workspace!.activeProjectId;
  snapshot = await service.save(owner, snapshot.revision, saveProjectTemplate(snapshot.workspace!, sourceId, "Application onboarding"));
  const template = structuredClone(snapshot.workspace!.templates[0]);
  const { templates: _templates, ...oldClient } = snapshot.workspace!;
  oldClient.projects[0].roadmap.title = "Older client edit";
  snapshot = await service.save(owner, snapshot.revision, oldClient);
  assert.deepEqual(snapshot.workspace!.templates, [template]);
  await service.saveMembers(owner, snapshot.revision, [...snapshot.members!, { email: editor.email, role: "editor", projectIds: [sourceId] }, { email: viewer.email, role: "viewer", projectIds: null }]);
  const scoped = await service.get(editor);
  assert.deepEqual(scoped.workspace!.templates, []);
  const attack = saveProjectTemplate(scoped.workspace!, sourceId, "Forbidden template");
  await assert.rejects(service.save(editor, scoped.revision, attack), status(403));
  const allowed = structuredClone(scoped.workspace!);
  allowed.projects[0].roadmap.title = "Scoped edit";
  await service.save(editor, scoped.revision, allowed);
  assert.deepEqual((await service.get(owner)).workspace!.templates, [template]);
  const view = await service.get(viewer);
  await assert.rejects(service.save(viewer, view.revision, saveProjectTemplate(view.workspace!, sourceId, "Viewer template")), status(403));
  snapshot = await service.get(owner);
  await service.saveMembers(owner, snapshot.revision, snapshot.members!.map(m => m.email === editor.email ? { ...m, projectIds: null } : m));
  const full = await service.get(editor);
  const saved = await service.save(editor, full.revision, saveProjectTemplate(full.workspace!, sourceId, "Editor template"));
  assert.equal(saved.workspace!.templates.length, 2);
  await assert.rejects(service.save(owner, full.revision, full.workspace), status(409));
});
