...
export const templateSchema = z.object({
  id: z.string().min(1).max(100), name: z.string().trim().min(1).max(100),
  description: z.string().trim().max(500), sourceProjectName: z.string().trim().min(1).max(100),
  roadmap: roadmapSchema,
});
...
export type RoadmapTemplate = z.infer<typeof templateSchema>;
export type Workspace = Omit<z.infer<typeof workspaceSchema>, "teams"> & { teams: Team[] };
export type ImportedProjects = Pick<Workspace, "engineers" | "projects" | "teams"> & { templates?: RoadmapTemplate[] };
...
export function validateWorkspace(input: unknown): Workspace {
  ...
  if (!parsed.success) throw new Error("Check the project names, templates, teams, engineers, and roadmaps. A workspace supports up to 100 projects, 100 templates, 500 teams, and 500 engineers.");
  ...
  if (!unique(workspace.templates.map(t => t.id))) throw new Error("Each template needs a unique ID.");
  if (!unique(workspace.templates.map(t => nameKey(t.name)))) throw new Error("A template with that name already exists. Choose a different name.");
  ...
  const templates = workspace.templates.map(template => {
    if (template.roadmap.tasks.some(t => t.status !== "todo" || t.blockedReason || t.assigneeIds?.length || t.decision?.answer)) throw new Error("Templates must have fresh progress, unanswered decisions, and no engineer assignments.");
    const roadmap = validateRoadmap(template.roadmap);
    roadmap.tasks = roadmap.tasks.map(task => {
      const team = task.teamId === undefined ? teams.find(t => nameKey(t.name) === nameKey(task.owner)) : teams.find(t => t.id === task.teamId);
      if ((task.teamId || (task.teamId === undefined && task.owner.trim())) && !team) throw new Error(`Choose an existing responsible team for “${task.title}” in template “${template.name}”.`);
      return { ...task, teamId: team?.id ?? null, owner: team?.name ?? "" };
    });
    return { ...template, roadmap };
  });
  return { ...workspace, teams, projects, templates };
}
...
export function saveProjectTemplate(workspace: Workspace, projectId: string, name: string, description = ""): Workspace {
  const project = workspace.projects.find(p => p.id === projectId);
  if (!project) throw new Error("This project no longer exists.");
  if (!name.trim()) throw new Error("Enter a template name.");
  if (workspace.templates.some(t => nameKey(t.name) === nameKey(name))) throw new Error("A template with that name already exists. Choose a different name.");
  const template: RoadmapTemplate = { id: crypto.randomUUID(), name: name.trim(), description: description.trim(),
    sourceProjectName: project.roadmap.application, roadmap: copyRoadmap(project.roadmap, true) };
  return validateWorkspace({ ...workspace, templates: [...workspace.templates, template] });
}
...
export function mergeProjectImport(workspace: Workspace, incoming: ImportedProjects): Workspace {
  ...
  function importRoadmap(source: Roadmap) {
    const roadmap = copyRoadmap(source, false);
    return { ...roadmap, tasks: roadmap.tasks.map(t => ({ ...t, teamId: t.teamId ? teamMap.get(t.teamId)! : null, assigneeIds: (t.assigneeIds ?? []).map(id => engineerMap.get(id)!) })) };
  }
  const imported = incoming.projects.map(p => {
    return { id: crypto.randomUUID(), engineerIds: p.engineerIds.map(id => engineerMap.get(id)!),
      roadmap: importRoadmap(p.roadmap),
    };
  });
  const templates = [...workspace.templates];
  for (const template of incoming.templates ?? []) {
    let name = template.name, suffix = 1;
    while (templates.some(t => nameKey(t.name) === nameKey(name))) name = `${template.name.slice(0, 80)} (imported ${suffix++})`;
    templates.push({ ...template, id: crypto.randomUUID(), name, roadmap: importRoadmap(template.roadmap) });
  }
  return validateWorkspace({ ...workspace, engineers, teams, templates, projects: [...workspace.projects, ...imported], activeProjectId: imported[0].id });
}
