...
export class WorkspaceStore {
  ...
  private async authorized(user: Identity): Promise<{ state: State; member: Member }> {
    for (let attempt = 0; attempt < 5; attempt++) {
      ...
      if (!state) {
        if (!this.adminEmail) throw new WorkspaceError(503, "Workspace setup is incomplete. The local owner has not been configured.");
  ...
  private present(state: State, member: Member, user: Identity): SharedSnapshot {
    ...
    return {
      workspace: projects.length ? { ...state.workspace, projects, activeProjectId: projects[0].id,
        templates: allProjects ? state.workspace.templates : [],
  ...
  async save(user: Identity, revision: number, input: unknown, supportsDecisions = true) {
    ...
    let next: Workspace;
    // Older clients do not send templates. Preserve the library on their saves.
    const allProjects = member.role === "owner" || member.projectIds === null;
    try { next = validateWorkspace({ ...input, templates: "templates" in input ? input.templates : allProjects ? state.workspace.templates : [] }); } catch (error) { throw new WorkspaceError(400, error instanceof Error ? error.message : "Invalid workspace."); }
    ...
    if (!allProjects && next.templates.length) throw new WorkspaceError(403, "Workspace templates require access to all projects.");
    if (allProjects && state.workspace.templates.some(t => !next.templates.some(n => n.id === t.id))) throw new WorkspaceError(400, "Existing templates must be retained.");
    ...
    try { workspace = validateWorkspace({ ...state.workspace, projects, engineers, templates: allProjects ? next.templates : state.workspace.templates, teams: member.role === "owner" ? next.teams : state.workspace.teams }); }
