...
export default function ProjectDialog({ mode, workspace, onManageTeams, onSave, onClose }: {
  ...
  const savedTemplate = workspace.templates.find(t => `saved:${t.id}` === template);
  ...
  function save() {
    try {
      ...
      if (mode === "create" && template.startsWith("saved:") && !savedTemplate) throw new Error("This template is no longer available. Choose another starting point.");
      const source = savedTemplate?.roadmap ?? (template === "current" ? current.roadmap : { ...sampleRoadmap, tasks: sampleRoadmap.tasks.map(t => ({ ...t, teamId: workspace.teams.find(team => team.name.toLowerCase() === t.owner.toLowerCase())?.id ?? null })) });
      const project = mode === "create" ? createProject(source, name.trim(), title.trim(), members)
  ...
  return <Dialog open onOpenChange={open => { if (!open) onClose(); }}><DialogContent className="project-dialog sm:max-w-[580px]">
    ...
    <form onSubmit={event => { event.preventDefault(); save(); }} className="project-form">
      <div className="project-fields"><div><Label htmlFor="project-name">Project name</Label><Input id="project-name" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Customer Portal" required maxLength={100} autoFocus /></div>
        ...
        {mode === "create" && <div><Label htmlFor="project-template">Start from</Label><Select value={template} onValueChange={value => {
          setTemplate(value); setTitle(workspace.templates.find(t => `saved:${t.id}` === value)?.roadmap.title ?? (value === "current" ? current.roadmap.title : sampleRoadmap.title));
        }}><SelectTrigger id="project-template" className="w-full"><SelectValue /></SelectTrigger><SelectContent>
          <SelectGroup><SelectLabel>Starting points</SelectLabel><SelectItem value="current">Current roadmap — fresh progress</SelectItem><SelectItem value="standard">Standard onboarding — fresh progress</SelectItem></SelectGroup>
          {!!workspace.templates.length && <SelectGroup><SelectLabel>Saved templates</SelectLabel>{workspace.templates.map(t => <SelectItem key={t.id} value={`saved:${t.id}`}>{t.name}</SelectItem>)}</SelectGroup>}
        </SelectContent></Select>
          {savedTemplate && <div className="template-source"><strong>Saved from {savedTemplate.sourceProjectName}</strong>{savedTemplate.description && <p>{savedTemplate.description}</p>}</div>}
          <p className="field-hint">Steps and dependencies are copied. Progress, decision answers, and task assignments start fresh.</p>
          {!workspace.templates.length && <p className="field-hint">To add a named template, open a project and choose “Save as template”.</p>}
        </div>}
